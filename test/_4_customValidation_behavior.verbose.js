  ...
  schemaList.forEach(schemaType => {
    describe(`when testing a/an '${schemaType}' schema with false returning custom validation`, function () {
      it(`should return boolean value 'false'.`, function () {
        expect(v2(example.value[schemaType], example.schema_returnTrue[schemaType], {verbose: true}).success).to.equal(true);
      });
    });
    describe(`when testing a/an '${schemaType}' schema with false returning custom validation`, function () {
      it(`should return boolean value 'false'.`, function () {
        expect(v2(example.value[schemaType], example.schema_returnFalse[schemaType], {verbose: true}).success).to.equal(false);
      });
    });
  });
});